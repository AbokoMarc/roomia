...
function withRoom(booking) {
  const room = db.prepare('SELECT id, title, city, images FROM rooms WHERE id = ?').get(booking.room_id);
  const payment = db.prepare('SELECT * FROM payments WHERE booking_id = ? ORDER BY id DESC LIMIT 1').get(booking.id);
...
export async function handleBookings(req, res, urlPath) {
  ...
  if (urlPath === '/api/bookings' && req.method === 'POST') {
    ...
    notifyAdmins('nouvelle_reservation', 'Nouvelle réservation', `${user.name} a réservé "${room.title}" (${code})`, { booking_id: booking.id });
    return json(res, 201, { booking: withRoom(booking) });
  }
  ...
  if (urlPath === '/api/bookings/mine' && req.method === 'GET') {
    const user = requireAuth(req, res);
    if (!user) return;
    const bookings = db.prepare('SELECT * FROM bookings WHERE user_id = ? ORDER BY created_at DESC').all(user.id).map(withRoom);
  ...
  if (cancelMatch && req.method === 'PUT') {
    const user = requireAuth(req, res);
    if (!user) return;
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(cancelMatch[1]);
    if (!booking) return notFound(res);
    if (user.role !== 'admin' && booking.user_id !== user.id) return json(res, 403, { error: 'Non autorisé.' });
    db.prepare(`UPDATE bookings SET status = 'annulee' WHERE id = ?`).run(cancelMatch[1]);
    const updated = db.prepare('SELECT * FROM bookings WHERE id = ?').get(cancelMatch[1]);
    if (user.role === 'admin') {
      notifyClient(booking.user_id, 'reservation_annulee', 'Réservation annulée', `Votre réservation ${booking.code} a été annulée.`, { booking_id: booking.id });
    } else {
      notifyAdmins('reservation_annulee', 'Réservation annulée par le client', `${booking.code} a été annulée.`, { booking_id: booking.id });
    }
    return json(res, 200, { booking: withRoom(updated) });
  }
  ...
  if (statusMatch && req.method === 'PUT') {
    ...
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(statusMatch[1]);
    if (!booking) return notFound(res);
    db.prepare('UPDATE bookings SET status = ? WHERE id = ?').run(status, statusMatch[1]);
    const updated = db.prepare('SELECT * FROM bookings WHERE id = ?').get(statusMatch[1]);
    notifyClient(booking.user_id, 'reservation_maj', 'Mise à jour de votre réservation', `Votre réservation ${booking.code} est maintenant : ${status.replace('_', ' ')}.`, { booking_id: booking.id });
    return json(res, 200, { booking: withRoom(updated) });
  }
