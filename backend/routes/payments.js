...
export async function handlePayments(req, res, urlPath) {
  ...
  if (urlPath === '/api/payments/accounts' && req.method === 'GET') {
    const accounts = db.prepare('SELECT method, label, destination FROM payout_accounts WHERE active = 1').all();
  ...
  if (urlPath === '/api/payments' && req.method === 'POST') {
    ...
    if (status === 'valide') {
      db.prepare(`UPDATE bookings SET status = 'confirmee' WHERE id = ?`).run(booking_id);
      db.prepare(`UPDATE payments SET validated_at = datetime('now') WHERE id = ?`).run(info.lastInsertRowid);
      db.prepare(`UPDATE users SET loyalty_points = loyalty_points + ? WHERE id = ?`).run(Math.round(booking.total_price / 1000), user.id);
    }
  ...
  if (validateMatch && req.method === 'PUT') {
    ...
    const payment = db.prepare('SELECT * FROM payments WHERE id = ?').get(validateMatch[1]);
    if (!payment) return notFound(res);
    db.prepare(`UPDATE payments SET status = 'valide', admin_note = ?, validated_at = datetime('now') WHERE id = ?`).run(admin_note || null, validateMatch[1]);
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(payment.booking_id);
    db.prepare(`UPDATE bookings SET status = 'confirmee' WHERE id = ?`).run(booking.id);
    db.prepare(`UPDATE users SET loyalty_points = loyalty_points + ? WHERE id = ?`).run(Math.round(booking.total_price / 1000), booking.user_id);
    notifyClient(booking.user_id, 'paiement_valide', 'Paiement confirmé', `Votre paiement pour la réservation ${booking.code} est validé. Séjour confirmé !`, { booking_id: booking.id });
  ...
  if (rejectMatch && req.method === 'PUT') {
    ...
    if (!admin) return;
    const { admin_note } = await parseBody(req);
    const payment = db.prepare('SELECT * FROM payments WHERE id = ?').get(rejectMatch[1]);
    if (!payment) return notFound(res);
    db.prepare(`UPDATE payments SET status = 'echoue', admin_note = ? WHERE id = ?`).run(admin_note || null, rejectMatch[1]);
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(payment.booking_id);
    notifyClient(booking.user_id, 'paiement_rejete', 'Paiement refusé', `Votre paiement pour la réservation ${booking.code} n'a pas pu être vérifié. ${admin_note || ''}`, { booking_id: booking.id });
    return json(res, 200, { success: true });
  }
  ...
  if (urlPath === '/api/admin/payout-accounts' && req.method === 'GET') {
    const admin = requireAdmin(req, res);
    if (!admin) return;
    const accounts = db.prepare('SELECT * FROM payout_accounts').all();
    return json(res, 200, { accounts });
  }
