...
export async function handleAdminStats(req, res, urlPath) {
  if (urlPath === '/api/admin/stats' && req.method === 'GET') {
    ...
    const activeRooms = db.prepare(`SELECT COUNT(*) c FROM rooms WHERE status = 'disponible'`).get().c;
    ...
    const confirmedBookings = db.prepare(`SELECT COUNT(*) c FROM bookings WHERE status = 'confirmee'`).get().c;
    const totalRevenue = db.prepare(`
      SELECT COALESCE(SUM(amount), 0) s FROM payments WHERE status = 'valide'
    `).get().s;
    const pendingPayments = db.prepare(`SELECT COUNT(*) c FROM payments WHERE status = 'en_attente'`).get().c;
    const totalUsers = db.prepare(`SELECT COUNT(*) c FROM users WHERE role = 'client'`).get().c;
    ...
    return json(res, 200, {
      totalRooms, activeRooms, totalBookings, pendingBookings, confirmedBookings,
      totalRevenue, pendingPayments, totalUsers, revenueByMethod, topRooms,
    });
  }
  ...
  if (urlPath === '/api/admin/users' && req.method === 'GET') {
    const admin = requireAdmin(req, res);
    if (!admin) return;
    const users = db.prepare(`SELECT id, name, email, phone, role, loyalty_points, created_at FROM users ORDER BY created_at DESC`).all();
